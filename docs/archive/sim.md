# Исторический проект SIM на miniplex и стык с ORDER/TRAY

> **Статус: проектный/исторический документ, не текущий план.** В production
> сейчас используется `StubSim`; настоящий miniplex-мир ещё не подключён.
> Актуальное состояние и ближайшие задачи находятся в [`plan.md`](plan.md) и
> [`backlog.md`](backlog.md).

## Что здесь устарело

- `!join`/`!start` заменены на взятие заказа с доски `!взять`/`!take`.
...
- Плоская модель `IOrder.items: IMenuItem[]` заменена на `IOrderItem` и
  `SEALED/PENDING`.
- Статус `CANCELLED` заменён на `EXPIRED`; в коде используется
  `IRecipe.fillingOrder`.
- Шаги S0–S7 ниже описывают желаемую реализацию, а не уже выполненные этапы.

Детали интерфейса и будущей архитектуры сохранены для переиспользования после
решения задачи B12.
